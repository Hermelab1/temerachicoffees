...
<header className="bg-white 2xl:h-[8vh] xl:h-[8vh] lg:h-[8vh] md:h-[7vh] h-[8vh] shadow-custom absolute w-full top-0 z-[999] opacity-80 flex items-center justify-center text-center">
  <div className='flex w-full justify-between  md:mx-12 mx-4'> {/* Add max-width for centering */}
    <div className="lg:w-[13%] md:w-[25%] w-[45%] py-2">
      <img 
        src={logo} 
    ...
    <div className={`lg:flex hidden xl:gap-16 lg:gap-8 py-2`}>
      {nav.map((item, index) => (
        <Link 
          key={index}
          to={item.path}
    ...
    {/* Responsive submenu */}
    {navlist && (
      <div className="small text-justify lg:hidden w-full bg-[#0b785f] absolute top-[8vh] md:top-[14vh]  left-0 p-6">
        <ul>
          {nav.map((item, index) => (
            <li key={index} className="mb-2">
              <Link
                to={item.path}
                onClick={() => handleNavLinkClick(item.path)}
                className="text-white text-[1.1rem]">{item.text}
              </Link>
            </li>
          ))}
        </ul>
